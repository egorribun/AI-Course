...
class TestM1ServiceWorkerEmpirical(unittest.TestCase):
    ...
    def test_02_sw_precache_assets_exist_and_non_empty(self):
        ...
        for asset in asset_paths:
            ...
            self.assertTrue(file_path.exists(), f"Precache asset does not exist on disk: {asset} ({file_path})")
...
class TestM1ProgressHubUIEmpirical(unittest.TestCase):
    ...
    def test_08_global_progress_hub_export_button_strictly_removed(self):
        ...
        self.assertNotEqual(end_idx, -1, "#exam-simulator-container element must exist following progress hub")
        hub_slice = self.index_text[start_idx:end_idx]

        self.assertNotIn("\u042d\u043a\u0441\u043f\u043e\u0440\u0442", hub_slice, "'Р­РєСЃРїРѕСЂС‚' text must not appear anywhere in #global-progress-hub")
        self.assertNotIn("\U0001f4be", hub_slice, "'рџ’ѕ' icon must not appear in #global-progress-hub")
        self.assertNotIn("exportProgressJSON", hub_slice, "exportProgressJSON call must not be present in #global-progress-hub")
    ...
    def test_09_global_progress_hub_reset_button_and_stats_preserved(self):
        ...
        self.assertIn("\U0001f504 \u0421\u0431\u0440\u043e\u0441", hub_slice, "'рџ”„ РЎР±СЂРѕСЃ' button must be present in #global-progress-hub")
        self.assertIn("CourseTracker.resetProgress()", hub_slice, "resetProgress() handler must be wired to Reset button")
        self.assertIn('id="global-progress-fill"', hub_slice, "'global-progress-fill' bar element must exist")
