...
class TestLectureHeadingHierarchy:
    ...
    @pytest.mark.parametrize('lecture_file', sorted(list(LECTURES_DIR.glob('*.html'))), ids=lambda p: p.name)
    ...
    def test_section_4_has_normalized_h3_headings(self, lecture_file):
        ...
        for i, (tag, level, text, _) in enumerate(parser.headings):
            if level == 2 and ('4.' in text or 'Шаг 4' in text or 'Практика' in text or 'Архитектура' in text or 'Алгоритм' in text):
                sec4_idx = i
                break
...
class TestServiceWorkerPrecacheParity:
    ...
    def test_static_assets_contains_exam_html_and_exam_js(self):
        ...
        assert './exam.html' in assets or 'exam.html' in assets, 'exam.html not found in STATIC_ASSETS'
        assert './js/exam.js' in assets or 'js/exam.js' in assets, 'js/exam.js not found in STATIC_ASSETS'
        assert './index.html' in assets or 'index.html' in assets, 'index.html not found in STATIC_ASSETS'
        assert './js/tracker.js' in assets or 'js/tracker.js' in assets, 'js/tracker.js not found in STATIC_ASSETS'
    ...
    def test_every_static_asset_exists_on_disk(self):
        match = re.search(r"const STATIC_ASSETS\s*=\s*\[([\s\S]*?)\];", self.sw_content)
        raw_assets = match.group(1)
        asset_lines = [line.strip().strip(',').strip('"').strip("'")
                       for line in raw_assets.splitlines() if line.strip() and not line.strip().startswith('//')]
...
class TestJSConsolidationAndHardening:
    ...
    def test_tracker_js_safeGetJSON_type_guards(self):
        ...
        assert "typeof defaultVal === 'object'" in tracker_code or 'typeof defaultVal === "object"' in tracker_code
        assert "typeof defaultVal === 'string'" in tracker_code or 'typeof defaultVal === "string"' in tracker_code
        assert "typeof defaultVal === 'number'" in tracker_code or 'typeof defaultVal === "number"' in tracker_code
        assert "typeof defaultVal === 'boolean'" in tracker_code or 'typeof defaultVal === "boolean"' in tracker_code
...
class TestAdversarialNodeSuitesExecution:
    ...
    def test_run_adversarial_client_state_fuzzing(self):
        ...
        result = subprocess.run(['node', str(script_path)], capture_output=True, text=True, cwd=str(ROOT_DIR))
        assert result.returncode == 0, f'adversarial_client_state_fuzzing failed:\n{result.stdout}\n{result.stderr}'
    ...
    def test_run_adversarial_sw_m1(self):
        ...
        result = subprocess.run(['node', str(script_path)], capture_output=True, text=True, cwd=str(ROOT_DIR))
        assert result.returncode == 0, f'adversarial_sw_m1 failed:\n{result.stdout}\n{result.stderr}'
    ...
    def test_run_adversarial_harness(self):
        ...
        result = subprocess.run(['node', str(script_path)], capture_output=True, text=True, cwd=str(ROOT_DIR))
        assert result.returncode == 0, f'adversarial_harness failed:\n{result.stdout}\n{result.stderr}'
