...
def test_01_viewports_and_responsive_css_layout():
    ...
    for w, label in viewports:
        ...
        assert (
            (hdr == "HIDDEN" and dock == "VISIBLE")
            if is_mob
            else (hdr == "VISIBLE" and dock == "HIDDEN")
        )
    ...
    for hpath in all_html_files:
        ...
        if hpath.parent.name == "lectures":
            ...
            for lvl in checker.headings:
                if prev_level > 0:
                    assert lvl <= prev_level + 1, (
                        f"{hpath.name}: Heading skip detected from h{prev_level} to h{lvl}"
                    )
                prev_level = lvl
...
def test_04_service_worker_precache_assets_integrity():
    ...
    lines = [
        l.strip()
        for l in assets_match.group(1).splitlines()
