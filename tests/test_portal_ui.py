...
class TestPortalUI(unittest.TestCase):
    ...
    def test_03_global_progress_hub_export_button_removed_and_reset_preserved(self):
        ...
        self.assertNotIn("💾 Экспорт", hub_html, "💾 Экспорт button must be removed from #global-progress-hub")
        self.assertNotIn("exportProgressJSON", hub_html, "exportProgressJSON onclick handler must not be in #global-progress-hub")

        # Verify Reset button is present
        self.assertIn("🔄 Сброс", hub_html, "🔄 Сброс button must remain in #global-progress-hub")
        self.assertIn("CourseTracker.resetProgress()", hub_html, "resetProgress handler must remain in #global-progress-hub")

        # Verify progress bar and stat cards
        self.assertIn('id="global-progress-fill"', hub_html, "global-progress-fill must remain in #global-progress-hub")
        self.assertIn('id="stat-lecs-val"', hub_html, "stat-lecs-val must remain in #global-progress-hub")
        self.assertIn('id="stat-qas-val"', hub_html, "stat-qas-val must remain in #global-progress-hub")
        self.assertIn('id="stat-tasks-val"', hub_html, "stat-tasks-val must remain in #global-progress-hub")
