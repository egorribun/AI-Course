...
EXPECTED_LECTURES = [
    f"{i:02d}-" + name for i, name in enumerate([
        "intro-ml.html", "fcnn.html", "autodiff-pinn.html", "losses-mle.html",
        "cnn-layers.html", "cnn-architectures.html", "optimizers.html", "hyperparams.html",
        "metric-learning.html", "contrastive-ssl.html", "vae.html", "gan.html",
        "diffusion.html", "cv-tasks.html", "rnn-lstm.html", "attention-seq2seq.html",
        "transformers.html", "self-attention.html", "lstm-vs-transformer.html",
        "text-word2vec.html", "mt-bleu.html", "enc-dec.html", "rl-intro.html",
        "bellman.html", "vi-pi-mc.html", "td-qlearning.html", "policy-gradient.html",
        "actor-critic.html"
    ])
]
...
class AdversarialHyperlinkTests(unittest.TestCase):
    ...
    def test_index_card_links(self):
        ...
        for expected in EXPECTED_LECTURES:
            ...
            self.assertIn(expected_href, unique_lecture_links, f"Missing link to {expected_href} in index.html")
        ...
        for filename in EXPECTED_LECTURES:
            ...
            for href, _ in parser.links:
                if href.startswith("#") and len(href) > 1:
                    target_id = href[1:]
                    self.assertIn(target_id, parser.ids, f"{filename}: Broken anchor link href='{href}', id not found in page")
    ...
    def test_pill_counters_match_dom(self):
        for filename in EXPECTED_LECTURES:
            ...
            if qa_match:
                ...
                self.assertEqual(pill_qa_count, parser.qa_count,
            ...
            task_match = re.search(r"(\d+)\s*(?:задач|микро-задач|задачи|tasks)", pill_texts, re.IGNORECASE)
            if task_match:
                pill_task_count = int(task_match.group(1))
                self.assertEqual(pill_task_count, parser.task_count,
    ...
    def test_math_delimiters_and_braces(self):
        for filename in EXPECTED_LECTURES:
            ...
            for dm in display_math:
                ...
                self.assertEqual(open_b, close_b, f"{filename}: Unbalanced braces in display math: $${dm[:40]}...$$")
            ...
            for im in inline_math:
                ...
                self.assertEqual(open_b, close_b, f"{filename}: Unbalanced braces in inline math: ${im}$")
...
class AdversarialAlgorithmicStressTests(unittest.TestCase):
    ...
    def test_l04_conv2d_dimension_calculator(self):
        ...
        for _ in range(50):
            ...
            self.assertEqual(out.shape[2], h_out_theo, f"Height mismatch for config H_in={h_in}, K={k}, P={p}, S={s}, D={d}")
            self.assertEqual(out.shape[3], w_out_theo, f"Width mismatch for config W_in={w_in}, K={k}, P={p}, S={s}, D={d}")
    ...
    def test_l12_ddpm_marginal_properties(self):
        ...
        self.assertTrue((alpha_bar[1:] <= alpha_bar[:-1]).all(), "alpha_bar must be monotonically decreasing")
        self.assertLess(alpha_bar[-1].item(), 0.5, "alpha_bar_T should be significantly lower than 1.0")
    ...
    def test_l17_self_attention_causal_mask_and_scaling(self):
        ...
        for i in range(N):
            for j in range(i + 1, N):
                self.assertTrue((attn_weights[:, :, i, j] == 0.0).all(),
    ...
    def test_l23_l24_bellman_contraction_and_value_iteration(self):
        ...
        self.assertLessEqual(dist_TV, gamma * dist_V + 1e-9, "Bellman operator must be a gamma-contraction")
    ...
    def test_l26_reinforce_policy_gradient_monotonic_improvement(self):
        ...
        self.assertGreater(final_probs[2].item(), 0.65, "REINFORCE must increase probability of highest reward action")
