...
class TestChallenger1M1Empirical(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.all_docs = {
            "index.html": cls.index_content,
            "exam.html": cls.exam_content,
    ...
    def test_01_viewport_transition_desktop_rules(self):
        ...
        self.assertIn(
            "display: none", bnb_match.group(1), "Base .bottom-nav-bar must be display: none"
        )
    ...
    def test_02_viewport_transition_mobile_media_query_rules(self):
        ...
        mq_match = re.search(
            r"@media\s*\(\s*max-width:\s*767px\s*\)\s*\{([\s\S]*?)\n\}", self.style_content
        )
    ...
    def test_03_touch_target_sizes_in_style_css(self):
        ...
        self.assertTrue(
            "min-height: 44px" in bni_css
            or "min-height: 48px" in bni_css
            or "min-height:48px" in bni_css
        )
    ...
    def test_04_touch_target_attributes_across_all_30_pages(self):
        ...
        for doc_name, content in self.all_docs.items():
            ...
            self.assertIn(
                'id="course-progress-modal"', content, f"{doc_name} missing #course-progress-modal"
            )
            self.assertIn(
                'id="modal-progress-close"', content, f"{doc_name} missing #modal-progress-close"
            )
    ...
    def test_06_bottom_nav_routing_integrity_across_all_pages(self):
        ...
        # 1. index.html
        self.assertIn(
            '<button type="button" class="bottom-nav-item" id="nav-search-btn"', self.index_content
        )
        self.assertIn(
            '<a href="exam.html" class="bottom-nav-item" id="nav-exam-btn"', self.index_content
        )

        # 2. exam.html
        self.assertIn(
            '<a href="index.html?focus=search" class="bottom-nav-item" id="nav-search-btn"',
            self.exam_content,
        )
        ...
        for lec_name, content in self.lecture_contents.items():
            self.assertIn(
                ...
                content,
                f"{lec_name} search href must be ../index.html?focus=search",
            )
            self.assertIn(
                'href="../exam.html"', content, f"{lec_name} exam href must be ../exam.html"
            )
    ...
    def test_07_safe_area_insets_and_viewport_fit_cover(self):
        ...
        self.assertIn(
            "viewport-fit=cover", self.index_content, "index.html missing viewport-fit=cover"
        )
        self.assertIn(
            "viewport-fit=cover", self.exam_content, "exam.html missing viewport-fit=cover"
        )
