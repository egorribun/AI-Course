...
lecture_files = sorted(
    [f.name for f in LECTURES_DIR.glob("*.html") if re.match(r"^\d{2}-.*\.html$", f.name)]
)
...
def extract_all():
    ...
    for fname in lecture_files:
        ...
        qa_pattern = re.compile(
            r'<details\s+class=["\'][^"\']*?\bqa\b[^"\']*?["\']>(.*?)</details>', re.DOTALL
        )
        qas = []
        for m in qa_pattern.finditer(content):
            ...
            ans = (
                ans_m.group(1).strip()
                if ans_m
                else qa_html[summary_m.end() :].strip()
                if summary_m
                else qa_html
            )
            ...
            qas.append({"summary": clean_summary, "body": clean_ans, "raw_html": m.group(0)})
